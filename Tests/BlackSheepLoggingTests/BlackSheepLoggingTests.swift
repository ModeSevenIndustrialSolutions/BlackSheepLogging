//
//  BlackSheepLoggingTests.swift
//  BlackSheepLoggingTests
//
...
class BlackSheepLoggingTests: XCTestCase {
    ...
    func testLogVerifiesIfShouldLogOnAllDestinations() {
        ...
        XCTAssertEqual(dest1.shouldLogToLevel, BlackSheepLogging.Level.warning)
        XCTAssertEqual(dest2.shouldLogToLevel, BlackSheepLogging.Level.warning)
    ...
    func testLogCallsAllDestinations() {
        ...
        XCTAssertEqual(dest1.didSendToLevel, BlackSheepLogging.Level.warning)
        XCTAssertEqual(dest2.didSendToLevel, BlackSheepLogging.Level.warning)
    ...
    func testDestinationIntegration() {
        ...
        XCTAssertEqual(log.countDestinations(), 2)
        console2.format = "$L: $M"
        console2.minLevel = BlackSheepLogging.Level.debug
        log.verbose("a verbose hello from hopefully just 1 console!")
        log.debug("a debug hello from 2 different consoles!")

        // add file
        let file = FileDestination()
        file.logFileURL = URL(string: "file:///tmp/testBlackSheepLogging.log")!
        XCTAssertTrue(log.addDestination(file))
        XCTAssertEqual(log.countDestinations(), 3)
        ...
        file2.logFileURL = URL(string: "file:///tmp/testBlackSheepLogging2.log")!
        console2.format = "$L: $M"
        file2.minLevel = BlackSheepLogging.Level.debug
    ...
    func testVersionAndBuild() {
        XCTAssertGreaterThan(BlackSheepLogging.version.length, 4)
        XCTAssertGreaterThan(BlackSheepLogging.build, 500)
    }

    func testStripParams() {
        var f = "singleParam"
        XCTAssertEqual(BlackSheepLogging.stripParams(function: f), "singleParam()")
        f = "logWithParamFunc(_:foo:hello:)"
        XCTAssertEqual(BlackSheepLogging.stripParams(function: f), "logWithParamFunc()")
        f = "aFunc()"
        XCTAssertEqual(BlackSheepLogging.stripParams(function: f), "aFunc()")
    }
...
private class MockDestination: BaseDestination {
    var didSendToLevel: BlackSheepLogging.Level?
    ...
    override func send(_ level: BlackSheepLogging.Level, msg: String, thread: String, file: String, function: String, line: Int, context: Any?) -> String? {
    ...
    var shouldLogToLevel: BlackSheepLogging.Level?
    var shouldLogPath: String?
    var shouldLogFunction: String?
    var shouldLogMessage: String?
    override func shouldLevelBeLogged(_ level: BlackSheepLogging.Level, path: String, function: String, message: String?) -> Bool {
